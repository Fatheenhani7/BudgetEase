...
<<<<<<< HEAD
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div class="w-full md:w-auto flex-1 max-w-xl">
            <form action="{{ route('marketplace.index') }}" method="GET" class="flex gap-2">
                @if(request('category'))
                ...
                @if(request('search'))
                    <a href="{{ route('marketplace.index', array_merge(
                        request()->except('search'),
                        ['category' => request('category'), 'location' => request('location'), 'sort' => request('sort')]
                    )) }}" 
